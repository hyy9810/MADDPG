...
import torch
import os
from maddpg.actor_critic import Actor, Critic, SActor, SCritic
...
class MADDPG:
    ...
    def train(self, transitions):
        ...
        with torch.no_grad():
            ...
            # 训练时（暂时）拿全部的reward
            r = torch.mean(torch.cat([transitions['r_%d' % i].unsqueeze(-1) for i in range(self.args.n_agents)],-1),-1)  
...
class DDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        # create the network
        self.actor_network = SActor(args, agent_id).to(args.device)
        self.critic_network = SCritic(args).to(args.device)

        # build up the target network
        self.actor_target_network = SActor(args, agent_id).to(args.device)
        self.critic_target_network = SCritic(args).to(args.device)
        ...
        if not os.path.exists(self.model_path):
            os.mkdir(self.model_path)
        self.model_path = self.model_path + '/' + 'Sagent'
        if not os.path.exists(self.model_path):
            os.mkdir(self.model_path)
    ...
    def train(self, transitions):
        ...
        o = torch.cat([transitions['o_%d' % agent_id] for agent_id in range(self.args.n_agents)],dim=1)
        u = torch.cat([transitions['u_%d' % agent_id] for agent_id in range(self.args.n_agents)],dim=1)
        o_next = torch.cat([transitions['o_next_%d' % agent_id] for agent_id in range(self.args.n_agents)],dim=1)
        ...
        writer.add_scalar("train/actor_loss", actor_loss.item(), self.train_step)
        writer.add_scalar("train/critic_loss", critic_loss.item(), self.train_step)
    ...
    def save_model(self, train_step):
        ...
        # if not os.path.exists(model_path):
        #     os.makedirs(model_path)
        model_path = self.model_path#os.path.join(model_path, 'agent')
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        torch.save(self.actor_network.state_dict(), model_path + '/' + num + '_actor_params.pkl')
        torch.save(self.critic_network.state_dict(),  model_path + '/' + num + '_critic_params.pkl')
